        ...
        if (not payloader.entity.valid) then
            ...
            if (not input_container.entity.valid or not output_container.entity.valid) then
                ...
                if (not input_inventory.is_empty() and internal_inventory.is_empty() and not payloader.entity.disabled_by_control_behavior) then
                    ...
                    local recipe_name = payloader_recipe_name or EMPTY_STRING
                    ...
                    --     --             if (payloader_signals[signals[i].signal.name or EMPTY_STRING]) then signal[payloader_signals[signals[i].signal.name]] = signals[i].count end
                    --     --         end
                    --     --     end
...
function locals.create_payloader(params)
    ...
    local ordered_payloaders_slice = ordered_payloaders[payloader.unit_number % NTH_TICK]
    ordered_payloaders_slice[#ordered_payloaders_slice+1] = payloader
end
