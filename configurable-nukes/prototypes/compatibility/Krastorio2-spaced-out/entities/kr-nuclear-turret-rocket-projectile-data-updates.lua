...
local area_multiplier = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.K2_SO_NUCLEAR_TURRET_ROCKET_AREA_MULTIPLIER.name })
local damage_multiplier = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.K2_SO_NUCLEAR_TURRET_ROCKET_DAMAGE_MULTIPLIER.name })
local repeat_multiplier = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.K2_SO_NUCLEAR_TURRET_ROCKET_REPEAT_MULTIPLIER.name })
...
-- Rocket PROJECTILE
-----------------------------------------------------------------------
local original_nuclear_turret_rocket = Util.table.deepcopy(data.raw["projectile"]["kr-nuclear-turret-rocket-projectile"])

local nuclear_turret_rocket = nil

local create_quality_nuclear_turret_rocket = function (params)

    local quality = params.quality
    local k_0 = params.quality_level

    if (k_0 ~= "quality-unknown" and not quality.hidden) then
        local quality_nuclear_turret_rocket = nil
        ...
        if (default_multiplier) then
            quality_nuclear_turret_rocket = Util.table.deepcopy(original_nuclear_turret_rocket)
            ...
            data:extend(
            {
                {
                    type = "projectile",
                    name = "kr-nuclear-turret-rocket-projectile-wave-spawns-nuke-shockwave-explosion" .. "-" .. k_0,
                    flags = {"not-on-map"},
                    hidden = true,
                    acceleration = 0,
                ...
                {
                    type = "projectile",
                    name = "kr-nuclear-turret-rocket-projectile-wave-spawns-fire-smoke-explosion" .. "-" .. k_0,
                    flags = {"not-on-map"},
                    hidden = true,
                    acceleration = 0,
                    speed_modifier = { 1, 0.707 },
            ...
            local nuclear_turret_rocket_ground_zero_projectile_action =
            {
                type = "area",
            ...
            local nuclear_turret_rocket_wave_action =
            {
                type = "area",
                radius = 3 * area_multiplier * quality_level_multiplier,
                ignore_collision_condition = true,
            ...
            nuclear_turret_rocket_wave_action.action_delivery =
            {
                type = "instant",
                target_effects = {
                    type = "damage",
            ...
            data:extend({
                {
                    type = "projectile",
                    name = "kr-nuclear-turret-rocket-projectile-ground-zero-projectile-" .. k_0,
                    flags = {"not-on-map"},
                    hidden = true,
                    acceleration = 0,
                    ...
                    action =
                    {
                        nuclear_turret_rocket_ground_zero_projectile_action
                    },
                    animation = nil,
                    shadow = nil
                },
                {
                    type = "projectile",
                    name = "kr-nuclear-turret-rocket-projectile-wave-" .. k_0,
                    flags = {"not-on-map"},
                    hidden = true,
                    acceleration = 0,
                    speed_modifier = { 1.0, 0.707 },
                    action =
                    {
                        nuclear_turret_rocket_wave_action
                    },
                    animation = nil,
                    shadow = nil
                },
            ...
            for k_1, v_1 in pairs(quality_nuclear_turret_rocket.action.action_delivery.target_effects) do
                if (v_1.type) then
                        ...
                        if (v_1.action.action_delivery and v_1.action.action_delivery.type == "projectile") then
                            if (v_1.action.action_delivery.projectile:find("atomic-bomb-ground-zero-projectile", 1, true)) then
                                v_1.action =
                                {
                                    ...
                                    {
                                        type = "projectile",
                                        projectile = "kr-nuclear-turret-rocket-projectile-ground-zero-projectile-" .. k_0,
                            ...
                            elseif (v_1.action.action_delivery.projectile:find("atomic-bomb-wave-spawns-fire-smoke-explosion", 1, true)) then
                                v_1.action = {
                                    ...
                                    action_delivery =
                                        {
                                            type = "projectile",
                                            projectile = "kr-nuclear-turret-rocket-projectile-wave-spawns-fire-smoke-explosion" .. "-" .. k_0,
                            ...
                            elseif (v_1.action.action_delivery.projectile:find("atomic-bomb-wave-spawns-nuke-shockwave-explosion", 1, true)) then
                                ...
                                action_delivery =
                                    {
                                        type = "projectile",
                                        projectile = "kr-nuclear-turret-rocket-projectile-wave-spawns-nuke-shockwave-explosion" .. "-" .. k_0,
                                        starting_speed = 0.5 * 0.65 * area_multiplier * quality_level_multiplier,
                                        starting_speed_deviation = nuke_shockwave_starting_speed_deviation * quality_level_multiplier,
                                    }
                                }
                            ...
                            elseif (v_1.action.action_delivery.projectile:find("atomic-bomb-wave", 1, true)) then
                                v_1.action = {
                                    ...
                                    {
                                        type = "projectile",
                                        projectile = "kr-nuclear-turret-rocket-projectile-wave-" .. k_0,
                                        starting_speed = 0.5 * 0.7 * area_multiplier * quality_level_multiplier,
                                        starting_speed_deviation = nuke_shockwave_starting_speed_deviation * quality_level_multiplier,
                                    }
                                }
            ...
            -- if (get_nuclear_turret_rocket_pollution()) then
                local target_effects = quality_nuclear_turret_rocket.action.action_delivery.target_effects
                        ...
                        action = {
                            action_delivery = {
                                target_effects = {
                                    {
                                        type = "script",
                                        effect_id = "k2-nuclear-turret-rocket-pollution"
                                    }
                                },
                                type = "instant"
                            },
                ...
                quality_nuclear_turret_rocket.action.action_delivery.target_effects = target_effects
            -- end

            if (Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.K2_SO_NUCLEAR_TURRET_ROCKET_FIRE_WAVE.name })) then
                local target_effects = quality_nuclear_turret_rocket.action.action_delivery.target_effects
                table.insert(target_effects,
                    {
                        type = "nested-result",
        ...
        if (quality_nuclear_turret_rocket ~= nil) then
            if (k_0 == "normal") then
                nuclear_turret_rocket = Util.table.deepcopy(quality_nuclear_turret_rocket)

                local nuclear_turret_rocket_ammo_item = data.raw["ammo"]["kr-nuclear-turret-rocket"]

                if (nuclear_turret_rocket_ammo_item.icon) then
                    nuclear_turret_rocket.icon = Util.table.deepcopy(nuclear_turret_rocket_ammo_item.icon)
                else
                    nuclear_turret_rocket.icons = Util.table.deepcopy(nuclear_turret_rocket_ammo_item.icons)
                end

                if (not nuclear_turret_rocket.icon and not nuclear_turret_rocket.icons) then
                    nuclear_turret_rocket.icon = "__Krastorio2Assets__/icons/ammo/nuclear-turret-rocket.png"
                end

                data:extend({nuclear_turret_rocket})
            end

            quality_nuclear_turret_rocket.name = quality_nuclear_turret_rocket.name .. "-" .. k_0
            data:extend({quality_nuclear_turret_rocket})

            return quality_nuclear_turret_rocket
        end
    end
end
...
if (mods and mods["quality"]) then
    for k_0, quality in pairs(data.raw["quality"]) do
        nuclear_turret_rocket = create_quality_nuclear_turret_rocket({ quality_level = k_0, quality = quality })
    end
else
...
nuclear_turret_rocket_placeholder.shadow = nil
nuclear_turret_rocket_placeholder.smoke = nil

local indices_to_remove = {}
for k, v in pairs(nuclear_turret_rocket_placeholder.action.action_delivery.target_effects) do
    ...
    elseif (v.type == "nested-result") then
        if (    v.action.action_delivery
            ...
            and v.action.action_delivery.projectile ~= "kr-nuclear-turret-rocket-projectile-ground-zero-projectile"
            and v.action.action_delivery.projectile ~= "kr-nuclear-turret-rocket-projectile-ground-zero-projectile-normal"
            and v.action.action_delivery.projectile ~= "kr-nuclear-turret-rocket-projectile-wave"
            and v.action.action_delivery.projectile ~= "kr-nuclear-turret-rocket-projectile-wave-normal"
        ) then
            table.insert(indices_to_remove, k)
        end
    end
...
for i = #indices_to_remove, 1, -1 do
    table.remove(nuclear_turret_rocket_placeholder.action.action_delivery.target_effects, indices_to_remove[i])
end


data:extend({nuclear_turret_rocket_placeholder})