...
local area_multiplier = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.SIMPLE_ATOMIC_ARTILLERY_SHELL_AREA_MULTIPLIER.name, })
local damage_multiplier = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.SIMPLE_ATOMIC_ARTILLERY_SHELL_DAMAGE_MULTIPLIER.name, })
local repeat_multiplier = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.SIMPLE_ATOMIC_ARTILLERY_SHELL_REPEAT_MULTIPLIER.name, })
local fire_wave = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.SIMPLE_ATOMIC_ARTILLERY_SHELL_FIRE_WAVE.name, })
...
-- Rocket PROJECTILE
-----------------------------------------------------------------------
local name = "atomic-artillery-projectile"
local original_nuclear_artillery_shell = Util.table.deepcopy(data.raw["artillery-projectile"]["atomic-artillery-projectile"])
...
--                 {
--                     type = "projectile",
--                     name = "atomic-artillery-projectile-wave-spawns-nuke-shockwave-explosion" .. "-" .. k_0,
--                     flags = {"not-on-map"},
--                     hidden = true,
--                     acceleration = 0,
...
--                 {
--                     type = "projectile",
--                     name = "atomic-artillery-projectile-wave-spawns-fire-smoke-explosion" .. "-" .. k_0,
--                     flags = {"not-on-map"},
--                     hidden = true,
--                     acceleration = 0,
--                     speed_modifier = { 1, 0.707 },
...
--                 {
--                     type = "projectile",
--                     name = "atomic-artillery-projectile-ground-zero-projectile-" .. k_0,
--                     flags = {"not-on-map"},
--                     hidden = true,
--                     acceleration = 0,
...
--                 {
--                     type = "projectile",
--                     name = "atomic-artillery-projectile-wave-" .. k_0,
--                     flags = {"not-on-map"},
--                     hidden = true,
--                     acceleration = 0,
--                     speed_modifier = { 1.0, 0.707 },
...
--                                     {
--                                         type = "projectile",
--                                         projectile = "atomic-artillery-projectile-ground-zero-projectile-" .. k_0,
...
--                                         {
--                                             type = "projectile",
--                                             projectile = "atomic-artillery-projectile-wave-spawns-fire-smoke-explosion" .. "-" .. k_0,
...
--                                     {
--                                         type = "projectile",
--                                         projectile = "atomic-artillery-projectile-wave-spawns-nuke-shockwave-explosion" .. "-" .. k_0,
--                                         starting_speed = 0.5 * 0.65 * area_multiplier * quality_level_multiplier,
--                                         starting_speed_deviation = nuke_shockwave_starting_speed_deviation * quality_level_multiplier,
--                                     }
--                                 }
...
--                                     {
--                                         type = "projectile",
--                                         projectile = "atomic-artillery-projectile-wave-" .. k_0,
--                                         starting_speed = 0.5 * 0.7 * area_multiplier * quality_level_multiplier,
--                                         starting_speed_deviation = nuke_shockwave_starting_speed_deviation * quality_level_multiplier,
--                                     }
--                                 }
...
--                                     {
--                                         type = "script",
--                                         effect_id = "saa-s-atomic-artillery-pollution"
--                                     }
--                                 },
--                                 type = "instant"
--                             },
...
--             if (Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.SIMPLE_ATOMIC_ARTILLERY_SHELL_FIRE_WAVE.name })) then
--                 local target_effects = quality_nuclear_artillery_shell.action.action_delivery.target_effects
--                 table.insert(target_effects,
--                     {
--                         type = "nested-result",
...
--                 local nuclear_artillery_shell_ammo_item = data.raw["ammo"]["atomic-artillery-shell"]
...
--                     nuclear_artillery_shell.icon = "__SimpleAtomicArtillery-S__/graphics/icons/atomic-artillery-shell.png"
--                 end
...
-- -- nuclear_artillery_shell = data.raw["artillery-projectile"]["atomic-artillery-projectile"]

-- local nuclear_artillery_shell_placeholder = Util.table.deepcopy(original_nuclear_artillery_shell)
-- nuclear_artillery_shell_placeholder.name = "saa-s-atomic-artillery-projectile-placeholder"
-- nuclear_artillery_shell_placeholder.action = nil
...
-- --             and v.action.action_delivery.projectile ~= "atomic-artillery-projectile-ground-zero-projectile"
-- --             and v.action.action_delivery.projectile ~= "atomic-artillery-projectile-ground-zero-projectile-normal"
-- --             and v.action.action_delivery.projectile ~= "atomic-artillery-projectile-wave"
-- --             and v.action.action_delivery.projectile ~= "atomic-artillery-projectile-wave-normal"
-- --         ) then
-- --             table.insert(indices_to_remove, k)
-- --         end
-- --     end
...
-- Rocket PROJECTILE
-----------------------------------------------------------------------
local name = "atomic-artillery-shell"
local original_munition = Util.table.deepcopy(data.raw["artillery-projectile"]["atomic-artillery-projectile"])
        ...
        if (quality_munition ~= nil) then
            if (k_0 == "normal" or quality.level == 0) then
                ...
                if (not munition.icon and not munition.icons) then
                    munition.icon = "__SimpleAtomicArtillery-S__/graphics/icons/atomic-artillery-shell.png"
                end
...
local p_tbls = {}
local to_remove = __Data_Utils.find_by(nuclear_artillery_shell_placeholder.action, "projectile",
    function (tbl, key, val, p_tbl)
        local ret_val = false

        if (type(tbl) ~= "table") then return end
        if (not key or not val) then return end

        if (tbl.type and projectile_types[tbl.type] and tbl.projectile) then
            if (tbl.projectile:find("%-explosion") or tbl.projectile:find("%-smoke")) then
                -- tbl.type = "instant"
                -- tbl.projectile = nil
                p_tbls[tbl] = p_tbl
                ret_val = true
...
-- log(serpent.block(p_tbls))
-- for k, v in pairs(p_tbls) do
--     log(serpent.block(k))
--     log(serpent.block(v))
-- end
-- log(serpent.block(to_remove))

for _, remove in ipairs(to_remove) do
    -- log(_)
    if (p_tbls[remove]) then
        -- log(serpent.block(remove))
        for _, p_tbl in pairs(p_tbls) do
            -- log(_)
            if (p_tbl == remove) then
                -- log(serpent.block(p_tbl))
                for i, tbl in pairs(p_tbl) do
                    -- log(i)
                    if (tbl == remove) then
                        -- log(i)
                        table.remove(p_tbl, i)
                    end
                end
            end
        end
...
nuclear_artillery_shell_placeholder.animation = nil
nuclear_artillery_shell_placeholder.picture = nil
nuclear_artillery_shell_placeholder.shadow = nil
nuclear_artillery_shell_placeholder.smoke = nil
