...
local get_payloader_research_prerequisites = function ()
    local setting = Startup_Settings_Constants.settings.PAYLOADER_RESEARCH_PREREQUISITES.default_value

    if (settings and settings.startup and settings.startup[Startup_Settings_Constants.settings.PAYLOADER_RESEARCH_PREREQUISITES.name]) then
        setting = settings.startup[Startup_Settings_Constants.settings.PAYLOADER_RESEARCH_PREREQUISITES.name].value
    end
    ...
    if (#prerequisites <= 0) then
        prerequisites = Startup_Settings_Constants.settings.PAYLOADER_RESEARCH_PREREQUISITES.prerequisites
    end

    return prerequisites
end
local get_payloader_research_ingredients = function ()
    local setting = Startup_Settings_Constants.settings.PAYLOADER_RESEARCH_INGREDIENTS.default_value

    if (settings and settings.startup and settings.startup[Startup_Settings_Constants.settings.PAYLOADER_RESEARCH_INGREDIENTS.name]) then
        setting = settings.startup[Startup_Settings_Constants.settings.PAYLOADER_RESEARCH_INGREDIENTS.name].value
    end
    ...
    if (#ingredients <= 0) then
        ingredients = Startup_Settings_Constants.settings.PAYLOADER_RESEARCH_INGREDIENTS.ingredients
    end

    return ingredients
end

--[[ payloader Technology ]]
...
data:extend({
    {
        type = "technology",
        name = "cn-payloader",
        icon = "__configurable-nukes__/graphics/technology/payloader.png",
        icon_size = 256,
        effects = technology_effects,
        prerequisites = get_payloader_research_prerequisites(),
        unit =
        {
            count = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.PAYLOADER_RESEARCH_COUNT.name }),
            ingredients = get_payloader_research_ingredients(),
            time = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.PAYLOADER_RESEARCH_TIME.name }),
        },
    },
})