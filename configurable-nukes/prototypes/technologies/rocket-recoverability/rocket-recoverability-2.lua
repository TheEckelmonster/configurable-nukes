...
local function get_rocket_recoverability_research_count()
    local setting = Startup_Settings_Constants.settings.ROCKET_RECOVERABILITY_2_RESEARCH_COUNT.default_value

    if (settings and settings.startup and settings.startup[Startup_Settings_Constants.settings.ROCKET_RECOVERABILITY_2_RESEARCH_COUNT.name]) then
        setting = settings.startup[Startup_Settings_Constants.settings.ROCKET_RECOVERABILITY_2_RESEARCH_COUNT.name].value
    end

    return setting
end
local function get_rocket_recoverability_research_prerequisites()
    local setting = Startup_Settings_Constants.settings.ROCKET_RECOVERABILITY_2_RESEARCH_PREREQUISITES.default_value

    if (settings and settings.startup and settings.startup[Startup_Settings_Constants.settings.ROCKET_RECOVERABILITY_2_RESEARCH_PREREQUISITES.name]) then
        setting = settings.startup[Startup_Settings_Constants.settings.ROCKET_RECOVERABILITY_2_RESEARCH_PREREQUISITES.name].value
    end
    ...
    -- end
    if (#prerequisites <= 0) then
        prerequisites = Startup_Settings_Constants.settings.ROCKET_RECOVERABILITY_2_RESEARCH_PREREQUISITES.prerequisites
    end

    return prerequisites
end
local get_rocket_recoverability_research_ingredients = function ()
    local setting = Startup_Settings_Constants.settings.ROCKET_RECOVERABILITY_2_RESEARCH_INGREDIENTS.default_value

    if (settings and settings.startup and settings.startup[Startup_Settings_Constants.settings.ROCKET_RECOVERABILITY_2_RESEARCH_INGREDIENTS.name]) then
        setting = settings.startup[Startup_Settings_Constants.settings.ROCKET_RECOVERABILITY_2_RESEARCH_INGREDIENTS.name].value
    end
    ...
    if (#ingredients <= 0) then
        ingredients = Startup_Settings_Constants.settings.ROCKET_RECOVERABILITY_2_RESEARCH_INGREDIENTS.ingredients
    end
...
local technology = {
    type = "technology",
    name = "cn-rocket-recoverability-2",
    -- icons = icons,
    icon = "__configurable-nukes__/graphics/technology/rocket-recoverability-2.png",
    icon_size = 256,
    localised_name = { "technology-name.cn-rocket-recoverability-2" },
    localised_description = { "technology-description.cn-rocket-recoverability-2" },
    ...
    unit =
    {
        ...
        time = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.ROCKET_RECOVERABILITY_2_RESEARCH_TIME.name }),
    },
}
