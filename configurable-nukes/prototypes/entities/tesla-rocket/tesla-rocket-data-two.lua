...
local original_tesla_rocket = require("prototypes.entities.tesla-rocket.tesla-rocket")
original_tesla_rocket.name = name

local original_rocket = require("prototypes.entities.tesla-rocket.tesla-rocket")
...
data:extend({ original_tesla_beam_chain_bounce, })
    ...
    if (k_0 ~= "quality-unknown" and not quality.hidden) then
        ...
        if (default_multiplier) then
            ...
            local cn_tesla_rocket_sticker_stun =
            {
                ...
                target_movement_modifier = 0.95 ^ quality_level_multiplier,
                ...
                if (v_1.type) then
                    ...
                    elseif (v_1.type == "create-sticker") then
                        if (v_1.sticker == "tesla-turret-stun") then
                            v_1.sticker = name .. "-stun"
                        elseif (v_1.sticker == "tesla-turret-slow") then
                            v_1.sticker = name .. "-slow"
                        end
            ...
            -- {
            --     type = "explosion",
            --     name = name .. "-lightning-bolt-explosion-" .. k_0,
            ...
            data:extend({
                {
                    ...
                    action =
                    {
                        {
                            type = "direct",
                            probability = 0.15 ^ (1 / quality_level_multiplier),
                            action_delivery =
                            {
                                type = "instant",
                                target_effects =
                                {
                                    ...
                                    },
                                    {
                                        ...
                                        action = {
                                            {
                                                type = "area",
                                                radius = clamp_max_distance(2.1 * ((area_multiplier * quality_level_multiplier) ^ 0.85)),
                                                action_delivery = {
                                                    type = "instant",
                                                    target_effects = {
                                                        {
                                                            damage = {
                                                                amount = 80 * damage_multiplier * quality_level_multiplier,
                                                                type = "electric"
                                                            },
                                                            type = "damage"
                                                        }
                                    ...
                                    --     entity_name = name .. "-lightning-bolt-explosion-" .. k_0,
            ...
            {
                type = "projectile",
                name = name .. "-wave-spawns-cluster-explosion-" .. k_0,
                flags = { "not-on-map" },
                hidden = true,
                acceleration = 0.001,
            ...
            table.insert(quality_tesla_rocket.action.action_delivery.target_effects,
            {
                ...
                action =
                {
                    ...
                    repeat_count = clamp_repeat_count(96 + 32 * repeat_multiplier * quality_level_multiplier),
                    radius = clamp_max_distance(8 + 8 * area_multiplier * quality_level_multiplier),
                    action_delivery =
                    {
                        type = "projectile",
                        projectile = name .. "-wave-spawns-cluster-explosion-" .. k_0,
            ...
            {
                type = "projectile",
                name = name .. "-ground-zero-projectile-" .. k_0,
                flags = { "not-on-map" },
                hidden = true,
                acceleration = 0,
                speed_modifier = { 1.0, 0.707 },
            ...
            table.insert(quality_tesla_rocket.action.action_delivery.target_effects,
            {
                ...
                action =
                {
                    ...
                    {
                        type = "projectile",
                        projectile = name .. "-ground-zero-projectile-" .. k_0,
            ...
            {
                type = "explosion",
                name = name .. "-shockwave-explosion-" .. k_0,
                icon = "__base__/graphics/icons/destroyer.png",
                flags = { "not-on-map" },
                hidden = true,
                subgroup = "explosions",
            ...
            {
                type = "projectile",
                name = name .. "-wave-spawns-nuke-shockwave-explosion-" .. k_0,
                flags = {"not-on-map"},
                hidden = true,
                acceleration = 0,
                ...
                {
                    {
                        ...
                        action_delivery =
                        {
                            ...
                            target_effects =
                            {
                                {
                                    type = "create-explosion",
                                    entity_name = name .. "-shockwave-explosion-" .. k_0,
                                    max_movement_distance = max_shockwave_movement_distance,
                                    max_movement_distance_deviation = max_shockwave_movement_distance_deviation,
                                    inherit_movement_distance_from_projectile = true,
                                    cycle_while_moving = true
            ...
            table.insert(quality_tesla_rocket.action.action_delivery.target_effects,
            {
                ...
                action =
                {
                    ...
                    action_delivery =
                    {
                        {
                            type = "projectile",
                            projectile = name .. "-wave-spawns-nuke-shockwave-explosion-" .. k_0,
        ...
        if (quality_tesla_rocket ~= nil) then
            if (k_0 == "normal") then
                ...
                local tesla_rocket_item = data.raw["ammo"][name]
                ...
                tesla_rocket.animation = nil
                tesla_rocket.shadow = nil
                tesla_rocket.smoke = nil
                tesla_rocket.action = nil
