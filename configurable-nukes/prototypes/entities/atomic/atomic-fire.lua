...
return function (name, quality_name, settings)
    ...
    local quality_level_multiplier = settings.quality_level_multiplier or 1
    local damage_multiplier = settings.damage_multiplier or 1
    local area_multiplier = settings.area_multiplier or 1
    local repeat_multiplier = settings.repeat_multiplier or 1

    local maximum_spread_count = settings.maximum_spread_count or 100
    local pollution = settings.pollution or 0.005
    local base_lifetime = settings.base_lifetime or 150
    local initial_lifetime = settings.initial_lifetime or nil
    local maximum_lifetime = settings.maximum_lifetime or nil

    num_levels = settings.num_levels or num_levels
    max_num_levels = settings.max_num_levels or max_num_levels
    min_num_levels = settings.max_num_levels or min_num_levels

    local __num_levels = num_levels
    local num_levels = clamp(math.ceil(__num_levels * quality_level_multiplier), max_num_levels, min_num_levels)
    ...
    for i = 1, num_levels, 1 do
        ...
        fire_flames[i] = __Data_Utils.table.merge({
            ...
            --[[ TODO: Add startup setting ]]
            emissions_per_second = { pollution = pollution * proportion_multiplier * quality_level_multiplier },

            --[[ TODO: Add startup settings ]]
            initial_lifetime =
                    initial_lifetime
                and (base_lifetime + (num_levels - i + 1) * initial_lifetime * quality_level_multiplier)
                or  (150 + (num_levels - i + 1) * (3 / E) * 60 * quality_level_multiplier),
            maximum_lifetime =
                    maximum_lifetime
                and (base_lifetime + num_levels * maximum_lifetime * quality_level_multiplier)
                or  (150 + num_levels * (E / 1.5) * 60 * quality_level_multiplier),
    ...
    local fire_sticker_duration = settings.fire_sticker_duration or nil
    ...
    for i = 1, num_levels, 1 do
        ...
        fire_stickers[i] = __Data_Utils.table.merge({
            ...
            duration_in_ticks = fire_sticker_duration
                and 150 + (num_levels - i + 1) * fire_sticker_duration * quality_level_multiplier
                or 150 + (num_levels - i + 1) * (3 / E) * 60 * quality_level_multiplier,
