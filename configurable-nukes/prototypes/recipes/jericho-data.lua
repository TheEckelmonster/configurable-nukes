...
--     return Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.JERICHO_INPUT_MULTIPLIER.name })
-- end
-- -- JERICHO_ADDITIONAL_CRAFTING_MACHINES
-- local get_jericho_additional_crafting_machines = function ()
--     local setting = Startup_Settings_Constants.settings.JERICHO_ADDITIONAL_CRAFTING_MACHINES.default_value

--     if (settings and settings.startup and settings.startup[Startup_Settings_Constants.settings.JERICHO_ADDITIONAL_CRAFTING_MACHINES.name]) then
--         setting = settings.startup[Startup_Settings_Constants.settings.JERICHO_ADDITIONAL_CRAFTING_MACHINES.name].value
--     end
...
-- local ingredients = {}
-- local jericho_recipe_string = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.JERICHO_RECIPE.name })
...
-- local i, j, param, param_val = jericho_recipe_string:find(search_pattern, 1)
-- local possible_matches = {}
-- local found_match = false
...
--     jericho_recipe_string = jericho_recipe_string:sub(j + 1, #jericho_recipe_string)

--     i, j, param, param_val = jericho_recipe_string:find(search_pattern, 1)
-- end

-- if (not Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.JERICHO_RECIPE_ALLOW_NONE.name })) then
...
--     if (#ingredients <= 0) then
--         ingredients = Startup_Settings_Constants.settings.JERICHO_RECIPE.ingredients
--         if (ingredients) then for k, v in pairs(ingredients) do v.amount = v.amount * get_input_multiplier() end end
--     end
-- end

-- local recipe_jericho =
-- {
--     type = "recipe",
--     name = "cn-jericho",
--     enabled = false,
--     requester_paste_multiplier = 1,
--     energy_required = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.JERICHO_CRAFTING_TIME.name }),
--     ingredients = ingredients,
--     results = {{ type = "item", name = "cn-jericho", amount = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.JERICHO_RESULT_COUNT.name }) }},
--     category = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.JERICHO_CRAFTING_MACHINE.name }),
--     additional_categories = get_jericho_additional_crafting_machines(),
--     auto_recycle = false,
-- }

-- data:extend({recipe_jericho})
...
local recipe =
{
    ...
    ingredients = Setting_Utils.get_recipe_ingredients({
        recipe_setting = Startup_Settings_Constants.settings.JERICHO_RECIPE,
    }) or Startup_Settings_Constants.settings.JERICHO_RECIPE.ingredients,
    -- results = {{ type = "item", name = "atomic-bomb", amount = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.JERICHO_RESULT_COUNT.name }) }},
    results = Setting_Utils.get_recipe_results({
        recipe_setting = Startup_Settings_Constants.settings.JERICHO_RESULTS,
    }) or Startup_Settings_Constants.settings.JERICHO_RESULTS.results,
    category = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.JERICHO_CRAFTING_MACHINE.name }),
    ...
    additional_categories = Setting_Utils.get_additional_crafting_machines({
        default_value = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.JERICHO_ADDITIONAL_CRAFTING_MACHINES.name }),
    }),
}
