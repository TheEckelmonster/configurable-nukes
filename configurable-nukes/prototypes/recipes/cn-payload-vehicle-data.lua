...
local get_input_multiplier = function ()
    return Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.PAYLOAD_VEHICLE_INPUT_MULTIPLIER.name })
end
-- PAYLOAD_VEHICLE_ADDITIONAL_CRAFTING_MACHINES
local get_payload_vehicle_additional_crafting_machines = function ()
    local setting = Startup_Settings_Constants.settings.PAYLOAD_VEHICLE_ADDITIONAL_CRAFTING_MACHINES.default_value

    if (settings and settings.startup and settings.startup[Startup_Settings_Constants.settings.PAYLOAD_VEHICLE_ADDITIONAL_CRAFTING_MACHINES.name]) then
        setting = settings.startup[Startup_Settings_Constants.settings.PAYLOAD_VEHICLE_ADDITIONAL_CRAFTING_MACHINES.name].value
    end
...
local i, j, param, param_val = payload_vehicle_recipe_string:find(search_pattern, 1)
local possible_matches = {}
local found_match = false
...
while param ~= nil and param_val ~= nil do
    ...
    payload_vehicle_recipe_string = payload_vehicle_recipe_string:sub(j + 1, #payload_vehicle_recipe_string)

    i, j, param, param_val = payload_vehicle_recipe_string:find(search_pattern, 1)
end

if (not Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.PAYLOAD_VEHICLE_RECIPE_ALLOW_NONE.name })) then
    ...
    if (#ingredients <= 0) then
        ingredients = Startup_Settings_Constants.settings.PAYLOAD_VEHICLE_RECIPE.ingredients
        if (ingredients) then for k, v in pairs(ingredients) do v.amount = v.amount * get_input_multiplier() end end
    end
end

local recipe_payload_vehicle =
{
    type = "recipe",
    ...
    results = {{ type = "item", name = "cn-payload-vehicle", amount = Data_Utils.get_startup_setting({ setting = Startup_Settings_Constants.settings.PAYLOAD_VEHICLE_RESULT_COUNT.name }) }},
    ...
    additional_categories = get_payload_vehicle_additional_crafting_machines(),
}

data:extend({recipe_payload_vehicle})