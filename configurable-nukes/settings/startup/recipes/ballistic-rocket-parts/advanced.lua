...
local recipes = {
    {
        setting_name = "ADVANCED_BALLISTIC_ROCKET_PART",
        name = "advanced-ballistic-rocket-part",
        energy_required = 30,
...
if (se_active) then
    ...
    {
        { type = "item", name = quantum_processor_prefix .. "quantum-processor", amount = math.floor((20 * (se_active and se_multiplier * 1.5 or 1)) / 1), },
        { type = "item", name = "low-density-structure", amount = math.ceil((36 * (se_active and se_multiplier * 1.5 or 1)) / 1), },
        { type = "item", name = "rocket-control-unit",   amount = math.ceil((36 * (se_active and se_multiplier * 1.5 or 1)) / 3), },
        { type = "item", name = "rocket-fuel",           amount = math.ceil((36 * (se_active and se_multiplier * 1.5 or 1)) / 1), },
        { type = "item", name = "uranium-fuel-cell",     amount = math.ceil((36) / 1), },
        { type = "item", name = "nuclear-fuel",          amount = math.ceil((6) / 1),  },
        { type = "item", name = "se-heat-shielding",     amount = math.ceil(36 * (se_multiplier * 1.5)) / 3 },
        { type = "item", name = "se-aeroframe-bulkhead", amount = math.floor(20 * (se_multiplier * 1.5)) / 3 },
    }
else
    table.insert(recipes[1].ingredients, { type = "item", name = "cn-heat-shielding", amount = math.ceil((24 * (se_active and se_multiplier * 1.5 or 1)) / 3), })
end
