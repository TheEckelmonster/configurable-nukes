...
local technology = {
    {
        setting = "ROCKET_RECOVERABILITY",
        name = "rocket-recoverability-1",
        count = {
            ["default"] = 1500,
            ["space-age"] = 1500,
            ["space-exploration"] = 1500,
        },
        research_time = 30,
        prerequisites = {
            "icbms",
...
if (cn_avionics_active or cn_materials_active or cn_propulsion_active) then
    if (cn_avionics_active) then
        count = count + 1
        table.insert(technology[1].prerequisites, "cn-avionics")
    end
    if (cn_materials_active) then
        count = count + 1
        table.insert(technology[1].prerequisites, "cn-materials-engineering")
    end
    if (cn_propulsion_active) then
        count = count + 1
        table.insert(technology[1].prerequisites, "cn-propulsion-systems")
    end
end
if (count < 3) then
    table.insert(technology[1].prerequisites, "cn-bral")
end
